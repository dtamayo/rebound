...
void problem_init(int argc, char* argv[]){
    ...
    double mass = input_get_double(argc,argv,"mass",1.e-4); // in jup masses
    ...
    for (int i=1;i<=Nplanets;i++){			// initialize the planets first, then initialize star so that center of mass is fixed at 0
		...
		struct particle p = tools_init_orbit3d(starmass, mass, a[i], e, inc, Omega, omega, f);
		particles_add(p);
	}
    ...
    output_double("Planet masses [mjup]",mass);
