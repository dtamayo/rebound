...
for taui in tauis:
    for sigmae in sigmaes:
        for q in range(numcores):
            of.write("qsub taui=%0.2e_sigmae=%0.2e_batch%d\n"%(taui,sigmae,q))
...
for taui in tauis:
    for sigmae in sigmaes:
        for q in range(numcores):
            of = open("taui=%0.2e_sigmae=%0.2e_batch%d"%(taui,sigmae,q), "w")
            of.write("#!/bin/bash\n")
            of.write("#PBS -l nodes=1:ppn=1\n")
            of.write("#PBS -q workq\n")
            of.write("#PBS -r n\n")
            of.write("#PBS -l walltime=12:00:00\n")
            of.write("#PBS -N taui=%0.2e_sigmae=%0.2e_batch%d\n"%(taui,sigmae,q))
            ...
            of.write("./taui=%0.2e_sigmae=%0.2e_%d\n"%(taui,sigmae,q))
            of.close()
            call("chmod u=rwx taui=%0.2e_sigmae=%0.2e_batch%d"%(taui,sigmae,q), shell=True)


        for q in range(numcores):
            of = open("taui=%0.2e_sigmae=%0.2e_%d"%(taui,sigmae,q), "w")
            of.write("#!/bin/bash\n")
            ...
            of.close()
            call("chmod u=rwx taui=%0.2e_sigmae=%0.2e_%d"%(taui,sigmae,q), shell=True)