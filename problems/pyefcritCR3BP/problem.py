...
def integrate(args):    
    Delta,ef,j,it,dr_thresh,folder = args
    ...
    p = pytools.kepler_particle(m=0.,primary=sun,a=a1,anom=random.uniform(0,2*math.pi),e=0.,omega=0.,inc=0.,Omega=0.)
    rebound.particle_add(p)
    p = pytools.kepler_particle(m=M,primary=sun,a=a2,anom=random.uniform(0,2*math.pi),e=0.,omega=0.,inc=0.,Omega=0.)
    rebound.particle_add(p)
    ...
    with open(folder + '/ef_{0:.5e}_Delta_{1:.5e}.txt'.format(ef,Delta), mode='a') as f:
        f.write("{0:.4e}\t{1:.4e}\n".format(ef,_t))

def main(argv):
    try:
        opts, args = getopt.getopt(argv, "D:e:j:", ["Delta=", "ef=", "j="])
    except getopt.GetoptError:
        print('Have to pass a Delta, ef and j')
        sys.exit(2)
    ...
    folder = "efCR3BP{0}to{1}".format(int(j+1), int(j))
    try:
        os.mkdir(folder)
    except OSError:
        pass
    ...
    args = []
    for it in range(24):
        args.append((Delta,ef,j,it,dr_thresh,folder))
