...
#endif // OPENGL
	init_boxwidth(150); 			// Using no boundary conditions now, so particles aren't removed beyond this dist.  This is just for graphics box size
...
void append_orbits(){
	...
	for (int i=1;i<N;i++){
		...
		fprintf(of,"%e\t%e\t%e\t%e\n",t,particles[i].x, particles[i].y, particles[i].z);
		fclose(of);
