...
void problem_init(int argc, char* argv[]){
    ...
    double taui = input_get_double(argc,argv,"taui",0);					// set to zero by default
    double k = input_get_double(argc,argv,"k",100);
    double sigma_i = input_get_double(argc,argv,"sigmai",0); 			// set to zero by default
	...
	star.m  = M[0];			// This is a sub-solar mass star
	particles_add(star);
	...
	for (int i=1;i<=Nplanets;i++){			// initialize the planets first, then initialize star so that center of mass is fixed at 0
		...
		struct particle p = tools_init_orbit3d(M[0], M[i], a[i], ecc[i], inc, Omega, omega, f);
		particles_add(p);

		tau_e[i] = taue;
		tau_i[i] = taui;
		if(i==1){ // only set the inner planet to have an outerward migration
			tau_a[i] = -taue*k;
		}
		else{
			tau_a[i] = 0;
